...
<%!
  public final static String VIEW = "../view/manodoperaModConduzioneView.jsp";
  public final static String INDIETRO = "../layout/manodoperaMod.htm";
  public final static String ELENCO = "../layout/manodopera.htm";
%>

<%

  String iridePageName = "manodoperaModConduzioneCtrl.jsp";
  ...
  try {
    ...
    if (hmManodopera.get("idTipoAttCompl") == null) {
      ...
      if(Validator.isNotEmpty(request.getParameter("elimina"))) {
        hmManodopera.put("idTipoAttCompl", new String[0]);
        manodoperaVO.setVDettaglioAttivita(null);
      }
    }
    else if (request.getParameterValues("idTipoAttCompl") != null) {
    ...
    else if (request.getParameterValues("desTipoAttCompl") == null) {
      //non ci sono elementi dalla stessa pagina
      if(Validator.isNotEmpty(request.getParameter("elimina"))) {
        hmManodopera.put("desTipoAttCompl", new String[0]);
      }
    ...
    else if (request.getParameterValues("descrizioneAttivitaCompl") == null){
      //non ci sono elementi dalla stessa pagina
      if(Validator.isNotEmpty(request.getParameter("elimina"))) {
        hmManodopera.put("descrizioneAttivitaCompl", new String[0]);
      }
    ...
    if (request.getParameter("salva") != null) {
      ...
      if (idTipoAttComplStr != null && idTipoAttComplStr.length > 0) {
        ...
        for (int i=0; i<idTipoAttComplStr.length; i++) {
          ...
          Integer(idTipoAttComplStr[i]),
          desTipoAttComplStr[i]));
      ...
      else 
      {
        ...
        try 
        {
          anagFacadeClient.updateManodopera(manodoperaVO, anagAziendaVO.getIdAzienda());
        }
        catch(SolmrException solmrExc) 
        {
          if (solmrExc.getMessage().startsWith("Non è possibile modificare contemporaneamente la data di fine")
              ||
              solmrExc.getMessage().startsWith("Impossibile inserire una data di fine validità")
              )
          {

	          errors = new ValidationErrors();
	          errors.add("dataFineValidita",new ValidationError(solmrExc.getMessage()));
	          request.setAttribute("errors", errors);
	          %>
