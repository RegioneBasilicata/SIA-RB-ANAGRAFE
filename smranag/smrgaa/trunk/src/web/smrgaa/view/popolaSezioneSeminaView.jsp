...
<%
	...
	java.io.InputStream layout = application.getResourceAsStream("/layout/popolaSezioneSemina.htm");
  ...
  try
  {
	  if(Validator.isNotEmpty(provenienza))
	  {
	    ...
	    else if("modificaTerreni".equalsIgnoreCase(provenienza))
	    {
	      ...
	      if(Validator.isNotEmpty(idUtilizzo) && Validator.isNotEmpty(idTipoDestinazione) 
		      && Validator.isNotEmpty(idTipoDettaglioUso) && Validator.isNotEmpty(idTipoQualitaUso)
		      && Validator.isNotEmpty(idVarieta) && Validator.isNotEmpty(idParticellaCertificata))
		    {
          ...
          htmpl.set("blkVariabiliMod.coefficienteRiduzione", Formatter.formatDouble2(catalogoMatriceVO.getCoefficienteRiduzione()));
          
          BigDecimal supEleggibile = anagFacadeClient.getSupEleggPlSqlTotale(
            new Long(idParticellaCertificata), new Long(catalogoMatriceVO.getIdCatalogoMatrice()));
          BigDecimal supEleggibileNetta = anagFacadeClient.getSupEleggNettaPlSqlTotale(
            new Long(idParticellaCertificata), new Long(catalogoMatriceVO.getIdCatalogoMatrice()));
        
          htmpl.set("blkVariabiliMod.supEleggibile", Formatter.formatDouble4(supEleggibile));
          htmpl.set("blkVariabiliMod.supEleggibileNetta", Formatter.formatDouble4(supEleggibileNetta));
