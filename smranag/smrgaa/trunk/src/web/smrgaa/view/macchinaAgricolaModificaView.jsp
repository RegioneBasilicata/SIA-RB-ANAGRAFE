...
<%
  java.io.InputStream layout = application.getResourceAsStream("/layout/macchinaAgricolaModifica.htm");
  ...
  String regimeMacchinaAgricolaModifica = request.getParameter("regimeMacchinaAgricolaModifica");
  String parametroModMaccLibera = (String)request.getAttribute("parametroModMaccLibera");
  boolean modificabile = false;
  String cambioUte = (String)request.getAttribute("cambioUte");
  ...
  if(Validator.isNotEmpty(cambioUte)) 
  {
    htmpl.set("cambioUte", cambioUte);
  }
  
  if((Validator.isNotEmpty(operazione) && operazione.equalsIgnoreCase("modificaUte"))
   || Validator.isNotEmpty(modificaUte))
  {
    htmpl.set("modificaUte", "modificaUte");
  }
  ...
  {
    String idUte = "";
    if(Validator.isNotEmpty(regimeMacchinaAgricolaModifica))
    { 
  ...
  {
    idTipoFormaPossesso = possessoMacchinaVO.getIdTipoFormaPossesso().toString();
  }
  ...
  {
    if(Validator.isNotEmpty(possessoMacchinaVO.getPercentualePossesso()))
    {
      htmpl.set("percentualePossesso", Formatter.formatDouble(possessoMacchinaVO.getPercentualePossesso()));
      if(!"S".equalsIgnoreCase(abilitaPercentualePossesso))
      {
  ...
  if((Validator.isNotEmpty(operazione) && operazione.equalsIgnoreCase("modificaUte"))
   || Validator.isNotEmpty(modificaUte))
  {
    htmpl.set("dataScarico", "");
    htmpl.set("readOnlyDataScarico", "readOnly=\"true\"", null);  
    idScarico = "";
    htmpl.set("disabledMotScarico", "disabled=\"disabled\"", null);
    
    htmpl.newBlock("blkDataCarico");
  }
  else
  {
    htmpl.newBlock("blkNoDataCarico");
    htmpl.set("blkNoDataCarico.dataCarico", DateUtils.formatDateNotNull(possessoMacchinaVO.getDataCarico()));
  ...
  {
    for(int i=0;i<vTipoScarico.size();i++) 
    {
      CodeDescription tipoScarico = vTipoScarico.get(i);
      htmpl.newBlock("blkTipoScarico");
      htmpl.set("blkTipoScarico.idScarico", ""+tipoScarico.getCode());
      htmpl.set("blkTipoScarico.descrizione", tipoScarico.getDescription());
      
      if(Validator.isNotEmpty(idScarico) 
        && new Integer(idScarico).intValue() == tipoScarico.getCode().intValue()) 
      {
        htmpl.set("blkTipoScarico.selected", "selected=\"selected\"",null);        
      }
    }
  }
