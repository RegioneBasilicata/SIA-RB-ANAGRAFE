...
<%
  ...
  String idDichiarazioneConsistenza = (String)request.getParameter("idDichiarazioneConsistenza");
 	...
 	java.io.InputStream layout = application.getResourceAsStream("/layout/popCercaParticellaCessata.htm");
  ...
  if(Validator.isNotEmpty(idDichiarazioneConsistenza))
    htmpl.set("idDichiarazioneConsistenza", idDichiarazioneConsistenza);
 	...
 	boolean hasUnitToDocument = true;
 	...
 	if(!Validator.isNotEmpty(messaggioErrore)) {
   		try 
      {
        if ("0".equals(idDichiarazioneConsistenza) || idDichiarazioneConsistenza==null)
     		   elencoParticelle = anagFacadeClient.getListParticelleForDocument(storicoParticellaVO, 
            anagAziendaVO, hasUnitToDocument, null, SolmrConstants.ORDER_BY_STORICO_PARTICELLA_LOGIC_KEY);
        else
          elencoParticelle = anagFacadeClient.getListParticelleForDocument(storicoParticellaVO, 
            anagAziendaVO, Long.parseLong(idDichiarazioneConsistenza), hasUnitToDocument, null, SolmrConstants.ORDER_BY_STORICO_PARTICELLA_LOGIC_KEY);
 	...
 	// Monitoraggio
 	watcher.dumpElapsed("popCercaParticellaCessataView.jsp", "Creation of list of particellaVO", "In popCercaParticellaCessataView.jsp view from the beginning to the end", "");
 	// STOP
 	watcher.stop();
