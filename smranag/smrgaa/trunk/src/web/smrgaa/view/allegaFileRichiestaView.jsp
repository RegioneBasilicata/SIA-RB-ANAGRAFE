...
<%
  java.io.InputStream layout = application.getResourceAsStream("/layout/allegaFileRichiesta.htm");
  ...
  String chiudiConferma = (String)request.getAttribute("chiudiConferma");
  if(errors.size() > 0)
  {
    chiudiConferma = "N";
  }
    
  htmpl.set("chiudiConferma", chiudiConferma);       
  
  AllegatoDocumentoVO allegatoDocumentoVO = (AllegatoDocumentoVO) session.getAttribute("allegatoDocumentoVO");
  htmpl.set("idDocumento", ""+allegatoDocumentoVO.getIdDocumento());
  //htmpl.set("nomeAllegato", allegatoDocumentoVO.getNomeLogico());
  
  Vector<AllegatoDocumentoVO> vElencoFileAllegati = (Vector<AllegatoDocumentoVO>) session.getAttribute("vElencoFileAllegati");
  ...
  SolmrLogger.info(this, " - allegaFileRichiestaView.jsp - FINE PAGINA");
%>
<%=htmpl.text()%>