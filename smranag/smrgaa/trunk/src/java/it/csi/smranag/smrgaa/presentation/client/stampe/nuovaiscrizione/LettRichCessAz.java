...
public class LettRichCessAz extends SubReport
{
  private final static String TEMPLATE_NAME     = "/it/csi/solmr/etc/anag/stampe/nuovaiscrizione/LettRichCessAz.srt";
  private final static String CODICE_SUB_REPORT = "LETT_RICH_CESS_AZ";

  public LettRichCessAz() throws IOException, SolmrException
  {
      ...
      throws Exception
  {
    ...
    String txtTestoDichiarazione = (String)anagFacadeClient.getValoreParametroAltriDati(SolmrConstants.PARAMETRO_STM_INTRO_DC);
    ...
    subReport.setElement("txtCessazione", StampeGaaServlet.checkNull(aziendaNuovaVO.getDescCessazione()));
    if(Validator.isNotEmpty(aziendaNuovaVO.getCuaaSubentrante()))
    {
      String datiSubentro = aziendaNuovaVO.getCuaaSubentrante()+" - "+aziendaNuovaVO.getDenominazioneSubentrante();
      subReport.setElement("txtDatiSubentro", StampeGaaServlet.checkNull(datiSubentro));
    }
    else
    {
      subReport.removeElement("txtLabelSubentro");
      subReport.removeElement("txtDatiSubentro");
      subReport.removeElement("nlSubentro");
    }
