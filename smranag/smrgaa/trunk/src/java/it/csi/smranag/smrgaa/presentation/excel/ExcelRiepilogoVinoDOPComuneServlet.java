 ...
 * @version 1.0
 */
public class ExcelRiepilogoVinoDOPComuneServlet extends ExcelServlet
{
   ...
   * 
   */
  private static final long serialVersionUID = -1242389136481287460L;
  ...
  private void process(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
  {
    try
    {
      SolmrLogger.debug(this, " - ExcelRiepilogoVinoDOPComuneServlet - INIZIO PAGINA ");
      ...
      if(Validator.isNotEmpty(idDichiarazioneConsistenza))
      {
        ...
        if(idDichiarazioneConsistenzaLg.longValue() <= 0)
        {
          elencoRiepiloghi = gaaFacadeClient
            .riepilogoDestinazioneProduttivaVinoDOP(anagAziendaVO.getIdAzienda().longValue());
        }
        else
        {
          elencoRiepiloghi = gaaFacadeClient
            .riepilogoDestinazioneProduttivaVinoDOPDichiarato(idDichiarazioneConsistenzaLg);
        }
      }
      ...
      int indiceColonne = 0;
      sheet.setColumnWidth((short)indiceColonne++,(short)7000); //Idoneità
      sheet.setColumnWidth((short)indiceColonne++,(short)7000); //Comune
      sheet.setColumnWidth((short)indiceColonne++,(short)7000); //Sup. vit.
      ...
      ExcelServlet.buildCell(row, styleHeader,"STAMPA RIEPILOGO UNITA' ARBOREE PER VINO DOP - COMUNE AGGIORNATO AL "+descrizionePiano,(short) 0);
      ...
      ExcelServlet.buildCell(row, styleHeaderTable,"Sup. vit.",(short) colonna++);
      ...
      if(Validator.isNotEmpty(elencoRiepiloghi))
      {
        for (int i=0;i<elencoRiepiloghi.size();i++)
        {
          ...
          ExcelServlet.buildCell(row, styleDatiTabellaDouble, Double.parseDouble(supVit.toString().replace(',', '.')),(short) colonna++);
        }
      }
      ...
      colonna++;
      ExcelServlet.buildCell(row, styleTotaliTabellaDouble, Double.parseDouble(totSupVit.toString().replace(',', '.')),(short) colonna++);
        

      OutputStream out = response.getOutputStream();

      String fileName="RiepilogoUV_"+anagAziendaVO.getCUAA()+"_VinoDOPComune";

      setResponseHeader(response,request,fileName);
      workBook.write(out);
      out.close();

      SolmrLogger.debug(this, " - ExcelRiepilogoVinoDOPComuneServlet - FINE PAGINA ");
    }
    catch(Exception ex)
    {
      request.setAttribute("errorReport",ex);
      SolmrLogger.fatal(this, "ExcelRiepilogoVinoDOPComuneServlet Exception  "+ex.toString());
