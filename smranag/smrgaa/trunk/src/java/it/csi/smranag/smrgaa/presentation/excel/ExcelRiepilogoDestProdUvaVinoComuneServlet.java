 ...
 * @version 1.0
 */
public class ExcelRiepilogoDestProdUvaVinoComuneServlet extends ExcelServlet
{
   ...
   * 
   */
  private static final long serialVersionUID = -4202618893258944286L;
  ...
  private void process(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
  {
    try
    {
      SolmrLogger.debug(this, " - ExcelRiepilogoDestProdUvaVinoComuneServlet - INIZIO PAGINA ");
      ...
      if(Validator.isNotEmpty(idDichiarazioneConsistenza))
      {
        ...
        if(idDichiarazioneConsistenzaLg.longValue() <= 0)
        {
          elencoRiepiloghi = gaaFacadeClient
            .riepilogoDestinazioneProduttivaUvaDaVino(anagAziendaVO.getIdAzienda().longValue());
        }
        else
        {
          elencoRiepiloghi = gaaFacadeClient
            .riepilogoDestinazioneProduttivaUvaDaVinoDichiarato(idDichiarazioneConsistenzaLg);
        }
      }
      ...
      sheet.setColumnWidth((short)indiceColonne++,(short)6000); //Sup. vit. - DOP
      sheet.setColumnWidth((short)indiceColonne++,(short)6000); //Sup. vit. - Vino da tavola
      sheet.setColumnWidth((short)indiceColonne++,(short)6000); //Sup. vit. - Totale
      ...
      ExcelServlet.buildCell(row, styleHeader,"STAMPA RIEPILOGO UNITA' ARBOREE PER DESTINAZIONE PRODUTTIVA: UVA DA VINO - COMUNE AGGIORNATO AL "+descrizionePiano,(short) 0);
      ...
      ExcelServlet.buildCell(row, styleHeaderTable,"Sup. vit. - DOP",(short) colonna++);
      ExcelServlet.buildCell(row, styleHeaderTable,"Sup. vit. - Vino da tavola",(short) colonna++);
      ExcelServlet.buildCell(row, styleHeaderTable,"Sup. vit. - Totale",(short) colonna++);
      ...
      if(Validator.isNotEmpty(elencoRiepiloghi))
      {
        for (int i=0;i<elencoRiepiloghi.size();i++)
        {
          ...
          ExcelServlet.buildCell(row, styleDatiTabellaDouble, Double.parseDouble(supVitDOP.toString().replace(',', '.')),(short) colonna++);
          ...
          ExcelServlet.buildCell(row, styleDatiTabellaDouble, Double.parseDouble(supVitVinoTav.toString().replace(',', '.')),(short) colonna++);
          BigDecimal supVitTot = elencoRiepiloghi.get(i).getSupVitata();
          totSupVitTot = totSupVitTot.add(supVitTot);
          ExcelServlet.buildCell(row, styleDatiTabellaDouble, Double.parseDouble(supVitTot.toString().replace(',', '.')),(short) colonna++);          
        }
      }
      ...
      ExcelServlet.buildCell(row, styleTotaliTabellaDouble, Double.parseDouble(totSupVitDOP.toString().replace(',', '.')),(short) colonna++);
      ExcelServlet.buildCell(row, styleTotaliTabellaDouble, Double.parseDouble(totSupVitVinoTav.toString().replace(',', '.')),(short) colonna++);
      ExcelServlet.buildCell(row, styleTotaliTabellaDouble, Double.parseDouble(totSupVitTot.toString().replace(',', '.')),(short) colonna++);
        

      OutputStream out = response.getOutputStream();

      String fileName="RiepilogoUV_"+anagAziendaVO.getCUAA()+"_DestProdUvaVinoComune";

      setResponseHeader(response,request,fileName);
      workBook.write(out);
      out.close();

      SolmrLogger.debug(this, " - ExcelRiepilogoDestProdUvaVinoComuneServlet - FINE PAGINA ");
    }
    catch(Exception ex)
    {
      request.setAttribute("errorReport",ex);
      SolmrLogger.fatal(this, "ExcelRiepilogoDestProdUvaVinoComuneServlet Exception  "+ex.toString());
