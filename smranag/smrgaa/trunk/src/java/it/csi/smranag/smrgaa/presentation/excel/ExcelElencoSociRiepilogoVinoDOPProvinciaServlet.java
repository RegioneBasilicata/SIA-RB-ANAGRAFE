 ...
 * @version 1.0
 */
public class ExcelElencoSociRiepilogoVinoDOPProvinciaServlet extends ExcelServlet
{
   ...
   * 
   */
  private static final long serialVersionUID = 2288194541169580087L;
  ...
  private void process(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
  {
    try
    {
      ...
      TreeMap<String,Vector<RiepiloghiUnitaArboreaVO>> tRiepiloghi = gaaFacadeClient
        .riepilogoElencoSociProvinciaVinoDOP(anagAziendaVO.getIdAzienda().longValue());
      
      String descrizionePiano = DateUtils.getCurrent(SolmrConstants.DATE_EUROPEAN_STANDARD_FORMAT);
      ...
      sheet.setColumnWidth((short)indiceColonne++,(short)6000); //DO
      ...
      //Formato per visualizzare correttamente le superfici
      ...
      //Visulizzazione totali due cifre decimali
      ...
      ExcelServlet.buildCell(row, styleHeader,"STAMPA RIEPILOGO ELENCO SOCI UNITA' ARBOREE PER VINO - PROVINCIA AGGIORNATO AL "+descrizionePiano,(short) 0);
      ...
      if(tRiepiloghi != null)
      {
        ...
        Collection<Vector<RiepiloghiUnitaArboreaVO>> c = tRiepiloghi.values();
        
        //obtain an Iterator for Collection
        Iterator<Vector<RiepiloghiUnitaArboreaVO>> itr = c.iterator();
         
        //iterate through TreeMap values iterator
        while(itr.hasNext())
        {
          Vector<RiepiloghiUnitaArboreaVO> elencoRiepiloghi = itr.next();
          row=sheet.createRow(nRiga++); //salto riga
          ...
          row=sheet.createRow(nRiga++);
          ExcelServlet.buildCell(row, styleHeader,"Cuaa socio:",(short) 0);
          ExcelServlet.buildCell(row, styleHeader, elencoRiepiloghi.get(0).getCuaaAzienda(),(short) 1);
          ...
          row=sheet.createRow(nRiga++);
          ExcelServlet.buildCell(row, styleHeader,"Denominazione socio:",(short) 0);
          ExcelServlet.buildCell(row, styleHeader,elencoRiepiloghi.get(0).getDenominazioneAzienda(),(short) 1);
        ...
        //Unisco le celle prima della resa
          ...
          ExcelServlet.buildCell(row, styleHeaderTable,"DO",(short) colonna++);
      ...
      String fileName="RiepilogoElencoSociUV_"+anagAziendaVO.getCUAA()+"_VinoProvincia";

      setResponseHeader(response,request,fileName);
      workBook.write(out);
      out.close();

      SolmrLogger.debug(this, " - ExcelElencoSociRiepilogoVinoDOPProvinciaServlet - FINE PAGINA ");
    }
    catch(Exception ex)
    {
      request.setAttribute("errorReport",ex);
      SolmrLogger.fatal(this, "ExcelElencoSociRiepilogoVinoDOPProvinciaServlet Exception  "+ex.toString());
