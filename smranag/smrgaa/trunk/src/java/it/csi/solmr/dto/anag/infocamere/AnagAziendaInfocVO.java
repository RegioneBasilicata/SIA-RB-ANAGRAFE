package it.csi.solmr.dto.anag.infocamere;
...
public class AnagAziendaInfocVO implements Serializable
{
	/**
	 * 
	 */
		...
		if (vetSedi!=null)
			for (int i=0;i<vetSedi.size();i++)
				if (vetSedi.get(i)!= null)
					...
					if (vetSedi.get(i).getProgrSede().getValue().equals("0"))
						return vetSedi.get(i);
		return null;
	}

	@SuppressWarnings("unchecked")
  public void loadAnagAAEPAziendaVO(Azienda azienda,
		  AnagAziendaVO anagAziendaVO,
			PersonaFisicaVO personaVO)
	{
		/*try
		{
			this.setDataInizioValiditaAAEP(DateUtils.formatDate(azienda.getDataInizioValidita()));
		}
		catch(Exception e) {}*/

		//     Codice fiscale azienda
		//  Azienda.codicefiscale
		this.setCodFisAAEP(azienda.getCodiceFiscale().getValue());
		...
		if(azienda.getRagioneSociale().getValue().length() > 1000) {
			this.setDenominazioneAAEP(eliminaSpazi(azienda.getRagioneSociale().getValue().substring(0, 1000).trim()));
		}
		else {
			this.setDenominazioneAAEP(eliminaSpazi(azienda.getRagioneSociale().getValue()));
		}
		...
		//      Partita iva azienda
		//  Azienda.partitiva
		this.setPartitaIVAAAEP(azienda.getPartitaIva().getValue());
		...
		//      Forma giuridica
		//  Azienda.descrizioneNaturaGiuridca
		...
		//  Azienda.idAteco2007 + Azienda.descrizioneAteco2007
		/*String codAteco = azienda.getCodATECO2007();
		if(Validator.isNotEmpty(codAteco))
		{
		  codAteco = codAteco.replaceAll("\\.", "");
		}
		this.setIdAtecoAAEP(codAteco);
		this.setDescrizioneAtecoAAEP(convertNull(codAteco) +" - " +convertNull(azienda.getDescrATECO2007()));*/
		...
		//  Confrontare solo Azienda.idAteco9 con db_tipo_attivita_ateco.codice a
    ...
    this.tipoSezioniUguale = confrontaSezioni(vTipoSezAAEP, anagAziendaVO.getvAziendaSezioni());
		...
		//     Numero rea
		//  Azienda.numerocciaa
		this.setNumeroREAAAEP(azienda.getNumeroCCIAA().getValue());
    ...
    this.dataIscrizioneRIUguale=confrontaStr(dataIscrizioneRIAAEP, dataIscrizioneRIAnagrafe);
		...
		//  Azienda.annocciaa
		//this.setAnnoIscrizioneAAEP(azienda.getAnnoCCIAA());
		...
		//  Azienda.nRegistroImprese
		this.setNumeroRegistroImpreseAAEP(azienda.getNRegistroImpreseCCIAA().getValue());
    ...
    this.pecUguale=confrontaStr(pecAAEP, pecAnagrafe);
		...
		//     Data fondazione
		//  Azienda.datacostituzioneazienda
		if (azienda.getDataCostituzione()!=null)
		{
			try
			{
				this.setDataFondazioneAAEP(azienda.getDataCostituzione().getValue());
			}
			catch(Exception e) {}
		}

		//     Data cessazione
		//  Azienda.data_cessazione
		if (azienda.getDataCessazione()!=null)
		{
			try
			{
				this.setDataCessazioneAAEP(azienda.getDataCessazione().getValue());
			}
			catch(Exception e) {}
		}
		...
		{
			//     Causale cessazione
			//  Azienda.descrizionecausalecessazione
			this.setCausaleCessazioneAAEP(azienda.getDescrizioneCausaleCessazione().getValue());
		}
		...
		//     Codice fiscale
		//  RappresentanteLegale.codicefiscale
		if (rappresentanteLegaleAAEP!=null && rappresentanteLegaleAAEP.getCodiceFiscale() != null)
		...
		//     Cognome
		//  RappresentanteLegale.cognome
		if (rappresentanteLegaleAAEP!=null && rappresentanteLegaleAAEP.getCognome() != null)
		...
		//     Nome
		//  RappresentanteLegale.nome
		if (rappresentanteLegaleAAEP!=null && rappresentanteLegaleAAEP.getNome() != null)
		...
		//     Sesso
		//  RappresentanteLegale.sesso
		if (rappresentanteLegaleAAEP!=null && rappresentanteLegaleAAEP.getSesso() != null)
		...
		if (rappresentanteLegaleAAEP!=null && rappresentanteLegaleAAEP.getDataNascita() != null)
		{
			try
			{
		...
		//  RappresentanteLegale.datanascita
		try
		{
		...
		//  Decodifica di RappresentanteLegale.codicecomunenascita
		...
		if (rappresentanteLegaleAAEP!=null && 
				rappresentanteLegaleAAEP.getTipoVia() != null &&
				rappresentanteLegaleAAEP.getIndirizzo() != null &&
				rappresentanteLegaleAAEP.getNumeroCivico() != null)
		...
		//  RappresentanteLegale.toponimo + indirizzo + numero civico + cap + decodifica codicecomuneresidenza
		...
		if (rappresentanteLegaleAAEP!=null){
		  if(rappresentanteLegaleAAEP.getCap() != null && rappresentanteLegaleAAEP.getCodComuneResidenza() != null){
			if (confrontaStr(rappresentanteLegaleAAEP.getCap().getValue(),personaVO.getResCAP()) && confrontaStr(rappresentanteLegaleAAEP.getCodComuneResidenza().getValue(),personaVO.getResComune()))
				this.rapLegResidenzaUguale=true;
			else 
				this.rapLegResidenzaUguale=false;
		  }	
		}
      ...
      Vector<AziendaSezioniVO> vAziendaSezioni)
  {
	...
	public void setRappresentanteLegale(RappresentanteLegale rappresentanteLegaleAAEP) {
